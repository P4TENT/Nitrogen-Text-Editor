...
def ModifyTaskButton(button: int):
    ...
    def configure_buttons(except_button, color):
        for i in range(1, 10):
            if i != except_button:
                globals()[f'Button{i}'].configure(fg_color=color)
    
    selected_color = "#667e8f"
    dark_mode_color = "#353839"
    nitro_mode_color = "#263137"
    default_color = "#C0C0C0"
    
    globals()[f'Button{button}'].configure(fg_color=selected_color)
    
    if ThemeMode == "Dark":
        configure_buttons(button, dark_mode_color)
    elif ThemeMode == "Nitro":
        configure_buttons(button, nitro_mode_color)
    else:
        configure_buttons(button, default_color)
